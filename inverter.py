...
class Inverter(nn.Module):
    def __init__(self, name, args, clean=True):
        ...
        self.args = args
        self.initial_step = 0
        self.prev_path = "./output/inverter_preview"
        self.model_path = "./output/inverter"
        if clean:
            self.clean()
        self.writer = SummaryWriter(comment='inverter', log_dir=args.path_tensor_log)
    ...
    def itr_train(self, params, reference):
        ...
        y_ = self.forward(reference) # Inverter use image as input
        loss = F.l1_loss(params, y_)
    ...
    def batch_train(self, cuda=False):
        ...
        """
        # rnd_input = torch.randn(self.args.batch_size, 3, 512, 512) #TODO fix this
        ...
        for step in progress:
            ...
            self.writer.add_scalar('inverter/loss', loss_, step)

            if (step + 1) % self.args.prev_freq == 0:
                path = "{1}/imit_{0}.jpg".format(step + 1, self.prev_path)
                # self.capture(path, images, y_, self.args.parsing_checkpoint, cuda)  # TODO: get pretrained imitator
                x = step / float(total_steps)
                lr = self.args.learning_rate * (x ** 2 - 2 * x + 1) + 2e-3
                utils.update_optimizer_lr(self.optimizer, lr)
                self.writer.add_scalar('inverter/learning rate', lr, step)
    ...
    def load_checkpoint(self, path, training=False, cuda=False):
        ...
        if not os.path.exists(path_):
            raise NeuralException("not exist checkpoint of inverter with path " + path_)
        if cuda:
        ...
        log.info("recovery inverter from %s", path)
        if training:
    ...
    def evaluate_model(self, cuda=False):
        ...
        Inverter Load
        """
        path_ = "./output/inverter_base" + "/" + "imitator_30000_cuda.pth"
        if not os.path.exists(path_):
            raise NeuralException("not exist checkpoint of imitator with path " + path_)
        if cuda:
            checkpoint = torch.load(path_)
        ...
        dataset = FaceDataset(self.args, mode="test")
        for i in range(self.args.eval_inverter):
            names, params, images = dataset.get_batch(batch_size=self.args.batch_size, edge=False)
            if cuda:
                params = params.cuda()
                images = images.cuda()

            fake_params = self.model(images) # Input image to model
    ...
    def plot(self, losses):
        ...
        plt.xlabel('step')
        plt.ylabel('loss')
        path = os.path.join(self.prev_path, "inverter.png")
        plt.savefig(path)
